...
detect_repo() {
    ...
    case "$repo" in
        # Frontend variations (Nareshka)
        front|frontend|client|web|app|ui)
            echo "front"
            ;;
        # Backend variations (Nareshka)
        back|backend|server|api|src|services)
            echo "back"
            ;;
        # Database
...
# Function to get type-check command for repo
get_typecheck_command() {
    local repo="$1"
    local project_root="$CLAUDE_PROJECT_DIR"
    local repo_path="$project_root/$repo"

    # Frontend: TypeScript with tsc
    if [[ "$repo" == "front" ]]; then
        if [[ -f "$repo_path/tsconfig.json" ]]; then
            # Check for Vite/React-specific tsconfig
            if [[ -f "$repo_path/tsconfig.app.json" ]]; then
                echo "cd $repo_path && npm run type-check"
            else
    ...
    # Backend: Python mypy
    if [[ "$repo" == "back" ]]; then
        if [[ -f "$repo_path/.mypy.ini" ]]; then
            echo "cd $repo_path && poetry run mypy . --config-file .mypy.ini"
            return
        fi
    fi
...
typecheck_cmd=$(get_typecheck_command "$repo")
...
if [[ -n "$typecheck_cmd" ]]; then
    echo "$repo:typecheck:$typecheck_cmd" >> "$cache_dir/commands.txt.tmp"
fi
