...
function matchesPathPattern(filePath, patterns) {
...
function checkFileContent(projectDir, filePath, contentPatterns) {
    try {
...
function getRecentlyEditedFiles(projectDir, sessionId) {
...
async function main() {
    try {
        ...
        for (const [skillName, config] of Object.entries(rules.skills)) {
            ...
            if (promptTriggers && !matched) {
                ...
                if (promptTriggers.keywords) {
                    const keywordMatch = promptTriggers.keywords.find(kw => prompt.includes(kw.toLowerCase()));
                    if (keywordMatch) {
                        matchedSkills.push({
                            name: skillName,
                            matchType: 'keyword',
        ...
        if (matchedSkills.length > 0) {
            ...
            const sortedSkills = matchedSkills.sort((a, b) => priorityOrder[a.config.priority] - priorityOrder[b.config.priority]);
